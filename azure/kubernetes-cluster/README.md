...
| <a name="input_ingress_application_gateway"></a> [ingress\_application\_gateway](#input\_ingress\_application\_gateway) | Configuration for the ingress application gateway plugin. | <pre>object({<br>    enabled      = optional(bool, false)<br>    gateway_id   = optional(string)<br>    gateway_name = optional(string)<br>    subnet_id    = optional(string)<br>    subnet_cidr  = optional(string)<br>  })</pre> | `{}` | no |
