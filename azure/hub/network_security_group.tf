...
module "subnets_network_security_group" {
  count  = var.create_subnets_network_security_group ? 1 : 0
  source = "../network-security-group"

  name                = var.subnets_network_security_group_name
...
resource "azurerm_storage_account" "flow_log_sa" {
  ...
  name                = var.flow_log.storage_account_name != null ? var.flow_log.storage_account_name : lower(replace("${module.subnets_network_security_group.name}flsa1", "/[-_]/", ""))
  location            = var.location
