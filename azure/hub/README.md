...
| <a name="input_virtual_network_gateway"></a> [virtual\_network\_gateway](#input\_virtual\_network\_gateway) | Configuration for virtual network gateway if enabled. | <pre>object({<br>    name                = string<br>    address_prefix      = string<br>    resource_group_name = optional(string)<br>    public_ip_name      = optional(string)<br>    generation          = optional(string, "Generation1")<br>    sku                 = optional(string, "VpnGw1")<br>    type                = optional(string, "Vpn")<br>    vpn_type            = optional(string, "RouteBased")<br>  })</pre> | `null` | no |
