locals {
  ...
  create_flow_log_log_analytics_workspace = try(var.flow_log_config.create_flow_log_log_analytics_workspace, false)

  flow_log_workspace_id = var.flow_log_config != null ? (
    local.create_flow_log_log_analytics_workspace ? azurerm_log_analytics_workspace.flow_log_law[0].workspace_id : var.flow_log_config.workspace_id
  ) : null

  flow_log_workspace_resource_id = var.flow_log_config != null ? (
    local.create_flow_log_log_analytics_workspace == true ? azurerm_log_analytics_workspace.flow_log_law[0].id : var.flow_log_config.workspace_resource_id
  ) : null
}